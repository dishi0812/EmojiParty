...
class EmojiBouncyView: UIView, UICollisionBehaviorDelegate {
    ...
    init() {
        ...
        animator = UIDynamicAnimator(referenceView: self); collisions = UICollisionBehavior(items: []);
        collisions.translatesReferenceBoundsIntoBoundary = true; collisions.collisionDelegate = self;
